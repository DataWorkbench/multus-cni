...
import (
	...
	"github.com/DataWorkbench/multus-cni/pkg/device_plugin/server"
	log "github.com/sirupsen/logrus"
	"gopkg.in/fsnotify.v1"
)

func main() {
	log.Info("multus-nic device plugin starting")
	multusNicServer := server.NewMultusNicServer()
	go multusNicServer.Run()

	// 向 kubelet 注册
	if err := multusNicServer.RegisterToKubelet(); err != nil {
